...
/**
 * @ngdoc method
 * @name $ionicConfigProvider#Main.maxPrefetch
 * @description Sets the maximum number of Main to prefetch from the templateUrls defined in
 ...
 * @usage
 * State Main are cached automatically, but you can optionally cache other Main.
 *
 * ```js
 ...
 *     $ionicConfigProvider.Main.prefetch(false);
 *
 ...
 *           'home-tab': {
 *             prefetchTemplate: false,
 *             templateUrl: "tabs-Main/home.html",
...
function($http, $templateCache, $timeout) {
  ...
  function run() {
    ...
    while (i < 4 && (template = toCache.pop())) {
      // note that inline Main are ignored by this request
      if (isString(template)) $http.get(template, { cache: $templateCache });
      i++;
    }
    // only preload 3 Main a second
    if (toCache.length) {
      $timeout(run, 1000);
    }
  }
 ...
 *
 * The ionNavView directive is used to render Main in your application. Each template
 ...
 *
 * Pages are loaded by the URLs given. One simple way to create Main in Angular is to put
